...
def command_data_add():
  ...
  _command_add('Program','hoget',"hoget(command_data['Program']['hoget']['run'])",'hoget {URL}','To access website addresses')
  ...
  _command_add('Program','license',"_file = open('./LICENSE','r')\nprint(_file.read())\n_file.close()",'license','To view the license')
  ...
  _command_add('Program','trans',"par_m = par(_mode,'trans').split(' ',1)[0]\npar_t = par(_mode,'trans').split(' ',1)[1]\ndata = eval({\"y\":\"translate.YouDao().trans(par_t,char(par_t)[1])\",\"g\":\"translate.Google().trans(par_t,char(par_t)[1])\"}[par_m])\ni = 0\ndata_type = type(data).__name__\nif data_type == 'dict':\n  for word,pos in data.items():\n    i+=1\n    print(f'\033[1;94m{i} \033[0m\033[95m{pos}{word}\033[0m')\nelif data_type == 'list':\n  for trans in data:\n    print(f'\033[1;94m{i} \033[0m\033[95m{trans}\033[0m')\nelse:\n  print(data)",'trans {sources} {word}','To translate the word into Chinese/English')
