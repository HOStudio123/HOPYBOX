...
update_log_add(
    "0.0.1",
    "May 2, 2022",
    "* This is the first version of HOPYBOX posted on PyPI, but not the first version of HOPYBOX, The first version was born around Jan 28, 2022",
)
...
update_log_add(
    "1.8.2", "Jun 22 2024", "* Fixed some known issues\n* Add some new commands"
)
update_log_add("1.8.4", "Jun 22 2024", "* Supports color text on Windows")
update_log_add("1.8.6", "Jun 28 2024", "* Fixed some known issues")
update_log_add(
    "1.8.7", "Jun 29 2024", "* Fixed some known issues\n* Add some new commands"
)
update_log_add(
    "1.9.0", "Jun 29 2024", "* Fixed some known issues\n* Add some new commands"
)
update_log_add(
    "1.9.1", "Jun 29 2024", "* Fixed some known issues\n* Add some new commands"
)
update_log_add(
    "1.9.2",
    "July 6 2024",
    ...
    try:
        if version == "all":
            for version_num in update_log_content:
                print(
                    f"\033[96mHOPYBOX {version_num} Update Data\033[0m\033[92m ({update_log_content[version_num]['date']})\033[0m\n\033[95m{update_log_content[version_num]['update']}"
                )
        else:
            print(
                f"\033[96mHOPYBOX {version} Update Data\033[0m\033[92m ({update_log_content[version]['date']})\033[0m\n\033[95m{update_log_content[version]['update']}"
            )
    except KeyError:
        error_cross(
            "NotFoundVersionError",
            "Command",
            "No update log found for this version",
            f"update {version}",
        )
...
def update_program():
    try:
        ...
        if update_version_number >= int(res.text):
            ...
            while True:
                result = ask_proceed(
                    "There is a latest stable version discovered from PyPI"
                )
                if result == True:
